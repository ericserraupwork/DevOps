## SERVER ENVIRONMENT SETUP ON AWS ECS(GCP compute engine)

#### Overview
This is way to config server with nginx for Django & React.js(Angular). First we have to create two service for 
Backend(port 8000) and frontend(port 3000).
And nginx do load balancing request to BE & FE.
You can go following way to config server environment in AWS EC2.
...
+ setting service for frontend and backend
    ...
    + Flask service
        ...
        + `sudo nano /etc/systemd/system/api.service`
            ...
            ```
            [Unit]
            Description=Gunicorn instance to serve for Sellercenter of lazada
            After=network.target
